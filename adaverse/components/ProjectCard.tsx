    ...
    >
      <div className="relative overflow-hidden rounded-md bg-neutral-100 dark:bg-neutral-900 transition-all duration-300 hover:shadow-xl">
        ...
        {project.image ? (
          ...
          <div className="relative w-full bg-neutral-200 dark:bg-neutral-800" style={{paddingBottom: '56.25%'}}>
            <div className="absolute inset-0 flex items-center justify-center">
              <Image className="h-16 w-16 text-neutral-400 dark:text-neutral-700" />
            </div>
          </div>
        )}
          ...
          {project.students && project.students.length > 0 && (
            <div className="mb-4 flex flex-wrap gap-2">
              {project.students.slice(0, 4).map((student) => (
                <span
                  key={student.id}
                  className="inline-block rounded-full border border-neutral-300 dark:border-white/10 bg-neutral-200 dark:bg-neutral-800 px-3 py-1 text-xs font-medium text-neutral-700 dark:text-neutral-300"
                >
                  {student.name}
                </span>
              ))}
              {project.students.length > 4 && (
                <span className="inline-block rounded-full border border-neutral-300 dark:border-white/10 bg-neutral-200 dark:bg-neutral-800 px-3 py-1 text-xs font-medium text-neutral-500 dark:text-neutral-400">
                  +{project.students.length - 4}
                </span>
              )}
            </div>
